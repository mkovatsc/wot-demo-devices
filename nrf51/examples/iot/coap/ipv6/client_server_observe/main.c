//Set the required functionality of the code here
#define FUNCTIONALITY										1  //1=SWITCH, 2=PIR, 3=PIR USING BUTTON 3
#define DEVICE_NAME                     "SWITCH_1"    // SWITCH_1   PIR_1 PIR_2                                    /**< Device name used in BLE undirected advertisement. */
#define EP_NAME													"ep=SWITCH1"; // SWITCH1    PIR1  PIR2
 ...
 *  The address provided below is a place holder.  */
 //2001:470:cafe::38b2:cf42
#define SERVER_IPV6_ADDRESS 0x20, 0x01, 0x04, 0x70, 0xCA, 0xFE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x38, 0xB2, 0xCF, 0x50        /**< IPv6 address of the server node. */
...
#define BUTTON_FOUR                     BSP_BUTTON_3
...
#define APP_DISABLE_LOGS                1                                                     /**< Disable debug trace in the application. */
...
//static const char             m_lights_name[] = "switch";
static const char             m_led3_name[]   = "status";

static const char             m_semantics_switch_name[] = "restdesc";
static const char             m_semantics_switch_status_name[]   = "statedesc";

static coap_resource_t        m_led3;
static coap_resource_t        m_semantic_sts;
static uint32_t               m_observer_sequence_num = 0;
static uint32_t               m_observer_sequence_num_statedesc = 0;
                                                               
static void app_coap_time_tick(iot_timer_time_in_ms_t);
static void notify_all_semantic_sts_subscribers(coap_msg_type_t type);
...
static void notify_all_semantic_sts_subscribers(coap_msg_type_t type)
{
    ...
    while (coap_observe_server_next_get(&p_observer, p_observer, &m_semantic_sts) == NRF_SUCCESS)
    {
        // Generate a message.
        ...
        err_code = coap_message_opt_uint_add(p_response, COAP_OPT_OBSERVE, m_observer_sequence_num_statedesc++);
        APP_ERROR_CHECK(err_code);

        err_code = coap_message_opt_uint_add(p_response, COAP_OPT_MAX_AGE, m_semantic_sts.expire_time);
        APP_ERROR_CHECK(err_code);

        char * response_str;
				bool ison = (bool)LED_IS_ON(LED_THREE);
        if(ison)
					response_str = SEMANTICS_SWITCH_STATUS_ON;
				else
					response_str = SEMANTICS_SWITCH_STATUS_OFF;
...
static int coap_szx_to_size(uint8_t szx)
{
	int size = 16;
	for(int j = 0; j<szx; j++)
		size = size * 2;
...
static void semantics_callback(coap_resource_t * p_resource, coap_message_t * p_request)
{
    coap_message_conf_t response_config;
    memset (&response_config, 0, sizeof(coap_message_conf_t));
    ...
    switch (p_request->header.code)
    {
        case COAP_CODE_GET:
        {
            ...
            else
            {
							...
							char *payload_str;
							//Depending on the selected resource (restdesc or swdesc) formulate the payload
							if(strcmp(p_resource->name, m_semantics_switch_name) == 0)
							{
								payload_str = SEMANTICS_SWITCH;
							}
							else
							{
								//payload for swdesc depends on the status of the switch..
								bool isOn = (bool)LED_IS_ON(LED_THREE);
								if(isOn)
									payload_str = SEMANTICS_SWITCH_STATUS_ON;
								else
									payload_str = SEMANTICS_SWITCH_STATUS_OFF;
							}
						...
						/* 0 1 2 3 4 5 6 7
							...
							uint8_t count = p_request->options_count;
							for(int i=0; i < count; i++)
							{
								//APPL_LOG("[APPL]: Registration succeeded! : %s\r\n", p_response->options[i].p_data);
								if(p_request->options[i].number == 23) //Blockwise transfer
								{
										int j = 0;
										while(j < p_request->options[i].length) {
											block2 <<= 8;
											block2 |= p_request->options[i].p_data[j++];
										}
										num = block2>>4;
										size = coap_szx_to_size(block2 & 0x07);
										if (size > 64) size = 64;
										break; //there can be only one BLOCK option
								}
							}
							
							offset = num * size;
									
							char response_part[size];
							
							// check end of data -> more bit
							uint8_t more = 1;							
							int payload_len = strlen(payload_str);
							more = (offset + size) < payload_len;
							
							//APPL_LOG("Block2 offset=%d num=%d size=%d len=%d more%d", offset, num, size, payload_len, more);
							strncpy(response_part, payload_str + offset, size);
							//memcpy(response_part, SEMANTICS_SWITCH + offset, size);
							
							block2 = num<<4 | more<<3 | (coap_log_2(size / 16) & 0x07);
							
							err_code = coap_message_opt_uint_add(p_response, 23, block2);
							APP_ERROR_CHECK(err_code);
							response_str = response_part;
...
/*
    static coap_resource_t light_switch;
    err_code = coap_resource_create(&light_switch, m_lights_name);
    APP_ERROR_CHECK(err_code);

    err_code = coap_resource_child_add(&root, &light_switch);
    APP_ERROR_CHECK(err_code);
*/
    ...
    err_code = coap_resource_child_add(&root, &m_led3);
    APP_ERROR_CHECK(err_code);
		
		
    static coap_resource_t semantics_switch;
    err_code = coap_resource_create(&semantics_switch, m_semantics_switch_name);
    APP_ERROR_CHECK(err_code);
		semantics_switch.permission      = (COAP_PERM_GET );
		semantics_switch.callback        = semantics_callback;
		semantics_switch.ct_support_mask = COAP_CT_MASK_PLAIN_TEXT;

    err_code = coap_resource_child_add(&root, &semantics_switch);
    APP_ERROR_CHECK(err_code);		
		
		//static coap_resource_t semantics_switch_status;
    err_code = coap_resource_create(&m_semantic_sts, m_semantics_switch_status_name);
    APP_ERROR_CHECK(err_code);
		m_semantic_sts.permission      = (COAP_PERM_GET | COAP_PERM_OBSERVE);
		m_semantic_sts.callback        = semantics_callback;
		m_semantic_sts.ct_support_mask = COAP_CT_MASK_PLAIN_TEXT;
		m_semantic_sts.max_age         = 15;

    err_code = coap_resource_child_add(&root, &m_semantic_sts);
    APP_ERROR_CHECK(err_code);
    ...
    if (status == NRF_SUCCESS)
    {
        ...
        if (p_response->header.code == COAP_CODE_204_CHANGED)
        {
            //m_blink_led_three = true;
				...
				else if (p_response->header.code == COAP_CODE_201_CREATED)
        {
						uint8_t count = p_response->options_count;
						for(int i=0; i < count; i++)
						{
							APPL_LOG("[APPL]: Registration succeeded! : %s\r\n", p_response->options[i].p_data);
							/*if(p_response->options[i].number == 8) //Option where the location is known.
							{
								//m_registration_location = p_response->options[1];
								//char buf[] 
							}*/
						}
...
enum RD_MESSAGE_KIND{REGISTER, DELETE};

/**@brief Function for interaction with Registration Server.
 *
 * @param[in]   pin_no         The pin number of the button pressed.
 * @param[in]   button_action  The action performed on button.
 */
static void send_rd_message(enum RD_MESSAGE_KIND messageKind)
{
    ...
    //if (button_action == APP_BUTTON_PUSH)
    {
        ...
        message_conf.type = COAP_TYPE_CON;
				...
				if(messageKind == REGISTER)
					message_conf.code = COAP_CODE_POST;
				else if(messageKind == DELETE)
					message_conf.code = COAP_CODE_DELETE;
				
				const char uri_part_rd[]  = "rd";
				const char uri_query_ep[]  = EP_NAME;
				//const char uri_query_lt[]  = "lt=30";
        ...
        switch (messageKind)
        {
            case REGISTER:
            {
                err_code = coap_message_opt_str_add(p_request, COAP_OPT_URI_PATH, (uint8_t *)uri_part_rd, strlen(uri_part_rd));
                APP_ERROR_CHECK(err_code);
							
							  err_code = coap_message_opt_str_add(p_request, COAP_OPT_URI_QUERY, (uint8_t *)uri_query_ep, strlen(uri_query_ep));
                APP_ERROR_CHECK(err_code);
            ...
            case DELETE:
            {
                ...
                err_code = coap_message_opt_str_add(p_request, COAP_OPT_URI_PATH, (uint8_t *)uri_query_ep + 3 , strlen(uri_query_ep + 3));
                APP_ERROR_CHECK(err_code);
...
{    
    ...
    {	
				...
				{
					send_rd_message(DELETE);
				}
				else if(pin_no == BUTTON_TWO)
				{
					send_rd_message(REGISTER);
				}
				#if(FUNCTIONALITY == 1)
				else if (pin_no == BUTTON_FOUR)
        {						
            LEDS_INVERT(LED_THREE);
            notify_all_led3_subscribers(COAP_TYPE_NON);
						notify_all_semantic_sts_subscribers(COAP_TYPE_NON);
...
static void buttons_init(void)
{
    ...
    static app_button_cfg_t buttons[] =
    {
        {BUTTON_ONE, false, BUTTON_PULL, button_event_handler},
				...
				#if(FUNCTIONALITY == 2)
					{BUTTON_THREE, false, NRF_GPIO_PIN_NOPULL, button_event_handler},
				#else
...
static void app_coap_time_tick(iot_timer_time_in_ms_t wall_clock_value)
{
    ...
    else
    {
		...
		//notify_all_semantic_sts_subscribers(COAP_TYPE_NON);
    static uint32_t msg_count_2 = 0;

    if (m_semantic_sts.expire_time <= (0 + OBSERVE_NOTIFY_DELTA_MAX_AGE))
    {
        m_semantic_sts.expire_time = m_semantic_sts.max_age;

        if (msg_count_2 % 4 == 0)
        {
            notify_all_semantic_sts_subscribers(COAP_TYPE_CON);
        }
        else
        {
            notify_all_semantic_sts_subscribers(COAP_TYPE_NON);
